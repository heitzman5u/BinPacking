	...
	private int firstFitPacking(){
		...
		for(int cptObj = 0 ; cptObj < listeObj.size() ; cptObj ++){
			if (listeObj.get(cptObj) <= tailleSac){
				...
				// Si on a des objets dans notre liste et que nous n'avons pas encore de sac, on en cr鳬 un.
				if(listeSacs.size() == 0){
				...
				// si l'obj ne rentre dans aucun sac existant, on cr馥r un nouveau sac et on le met dedans.
				if (sacTrouve == false){
					Sac sac = new Sac(tailleSac);
	...
	private int bestFitPacking(){
		...
		for(int cptObj = 0 ; cptObj < listeObj.size() ; cptObj ++){
			if (listeObj.get(cptObj) <= tailleSac){
				...
				// on recherche le 1er sac existant dont la capacit�, apres l'ajout du sac, sera la plus grande.
				boolean sacTrouve = false;
				...
				for (int i = 0 ; i < listeSacs.size() ; i++){
					...
					if (capaDispoSac >= listeObj.get(cptObj)){ //si l'obj rentre dans le sac alors ...
						if(capaDispoSac - listeObj.get(cptObj) > maxCapaDispoSac ){ // je regarde si la capacit� apres ajout est la plus gande parmis tous les sacs
